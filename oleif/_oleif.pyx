...
cdef class iForest:
    ...
    cdef int tree_index
    cdef int exlevel
    cdef __oleif.iForest* thisptr

    @cython.boundscheck(False)
    @cython.wraparound(False)
    def __cinit__ (self, int ntrees, int sample_size, int limit=0, int ExtensionLevel=0, int seed=-1):
        if ExtensionLevel < 0:
            raise Exception("Wrong Extension")
        self.thisptr = new __oleif.iForest (ntrees, sample_size, limit, ExtensionLevel, seed)
    ...
    @cython.boundscheck(False)
    @cython.wraparound(False)
    def addNextTree (self, np.ndarray[double, ndim=2] X not None):
        if not X.flags['C_CONTIGUOUS']:
            X = X.copy(order='C')
        self.size_X = X.shape[0]
        self.dim = X.shape[1]
        self.thisptr.addNextTree (<double*> np.PyArray_DATA(X), self.size_X, self.dim)
